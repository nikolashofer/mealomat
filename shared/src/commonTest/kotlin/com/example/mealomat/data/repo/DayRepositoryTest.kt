...
class DayRepositoryTest {
    ...
    fun setUp() {
        ...
        days = DayRepository(db, outbox, auth, clock, plan)
    }
    ...
    private fun itemOn(date: LocalDate) = days.day(date)!!.meals.single().items.single()

    @Test
    fun aDayProjectsThePlanVersionThatOwnsIt() = runTest {
        val v1 = plan.createPlan(Slot(monday, 0))
    ...
    fun aDateBeforeEveryVersionHasNoDay() = runTest {
        ...
        assertNull(days.day(LocalDate(2026, 6, 15)))
    }
    ...
    fun aPastDayKeepsTheAmountItWasEatenAt() = runTest {
        ...
        val v2 = plan.editablePlan(monday, Slot(nextMonday, 0))
        val copied = plan.items(v2).single()
    ...
    fun stateSurvivesALaterRevision() = runTest {
        ...
        plan.editablePlan(monday, Slot(nextMonday, 0))
    ...
    fun aDayNobodyTouchedHasNoStateRows() = runTest {
        ...
        assertEquals(1, days.day(monday)!!.meals.single().items.size, "but the day still renders")
    }
    ...
    fun aMealIsDoneWhenEveryLineIsReadyOrTicked() = runTest {
        ...
        assertTrue(!days.day(monday)!!.meals.single().isDone)
        days.markPrepped(monday, rice)
        assertTrue(!days.day(monday)!!.meals.single().isDone, "one item still to do")
    ...
    fun itemsUnderAComponentInheritItsPrepMode() = runTest {
        ...
        val items = days.day(monday)!!.meals.single().items.associateBy { it.ingredientId }
